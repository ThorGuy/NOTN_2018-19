      ...
      while (!(line = reader.readLine()).equals("end")) {
        ...
        while (!(line = reader.readLine()).equals("wait")) {
          ...
          if (motors.containsKey(args[0])) {
            ...
            motors.get(args[0]).setTargetPosition(Integer.parseInt(args[1]) + motors.get(args[0]).getCurrentPosition());
            //TODO:Possibly set power after "wait" is reached
            motors.get(args[0]).setPower(Double.parseDouble(args[2]));
          } else {
            //TODO:Possibly set position after "wait" is reached
            servos.get(args[0]).setPosition(Double.parseDouble(args[1]));
          }
        }
        ...
        while (wait) {
          ...
          wait = false;
          for (DcMotor motor : motors.values().toArray(new DcMotor[motors.size()])) {
            if (motor.isBusy()) {
              wait = true;
            } else {
              motor.setPower(0);
